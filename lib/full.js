...
async function urlEncodedBodyParser(req) {
    const parsed = new url_1.URLSearchParams(await micro_1.text(req));
    const params = {};
...
async function parseRequestPayload(request, router) {
    ...
    if (method === 'POST' || method === 'PUT' || method === 'PATCH') {
        ...
        request.body = await (bodyParsers[contentType] || micro_1.text)(request.req);
    }
}
    ...
    const wrapped = async function (req, res) {
        ...
        try {
            ...
            if (responseValidator) {
                ...
                if (!validator)
                    throw boom_1.internal('', { message: favo_1.validationMessagesFormatter.invalidResponseCode(code) });
                const valid = validator(response);
                if (!valid) {
                    throw boom_1.internal('', {
                        message: favo_1.validationMessagesFormatter.invalidResponse(code),
